var org_uengine_codi_mw3_knowledge_IWfNode_mindmap = function(objectId, className){
	...
	if(this.mw3Obj.parentId == rootNodeId){
		this.obj.find('td.#mm_LineL').eq(0).remove();
		this.obj.css('margin-left','30px')
	}
	if(this.obj.parent().prev().length < 1){
		this.obj.find('td.#mm_LineL').eq(0).css('background','url(images/waveStyle/mm_TopLine.gif) 0% 50% no-repeat');
	}else if(this.obj.parent().next().length < 1){
		this.obj.find('td.#mm_LineL').eq(0).css('background','url(images/waveStyle/mm_BottomLine.gif) 0% 50% no-repeat');
	}
	
	if(this.obj.parent().prev().length < 1 && this.obj.parent().next().length < 1){
		this.obj.find('td.#mm_LineL').eq(0).css('background','url(images/waveStyle/mm_RightLine.gif) 0% 50% no-repeat')
	}
}

org_uengine_codi_mw3_knowledge_IWfNode_mindmap.prototype = {
		getPrev : function(){
