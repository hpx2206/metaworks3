 ...
 * @version $Id: FormApprovalActivity.java,v 1.1 2012/02/13 05:29:13 sleepphoenix4 Exp $
...
//			동의시 상태 관리를 위하여 추가된 소스
			if (this.getParentActivity().getClass() == AllActivity.class) {
				ArrayList<Activity> faActs = ((AllActivity)this.getParentActivity()).getChildActivities();
				for (Activity faAct : faActs) {
					if( faAct instanceof FormApprovalActivity){
						if (Activity.STATUS_RUNNING.equals(faAct.getStatus(instance)) && !this.getTracingTag().equals(faAct.getTracingTag())) {
							...
							((FormApprovalActivity)faAct).setStatus(instance, Activity.STATUS_CANCELLED);
						}
					}
				}
