...
public class CompetitionNode extends Database<ICompetitionNode> implements ICompetitionNode {
	
	public final static String DEFAULT_COMPETITION_COUNT = "5";
	public final static String KNOWLEGE = "knowlege";
	public final static String COMPETITION = "compete";
	...
	public static ICompetitionNode findCompetition(Session session) throws Exception {
		return CompetitionNode.findCompetition(session, true);
	}
	
	public static ICompetitionNode findCompetition(Session session, boolean isMore) throws Exception {
		...
		if(!isMore)
			sb.append(" limit " + GlobalContext.getPropertyString("topic.more.count", DEFAULT_COMPETITION_COUNT));		
		
		ICompetitionNode dao = (ICompetitionNode)MetaworksDAO.createDAOImpl(TransactionContext.getThreadLocalInstance(), sb.toString(), ICompetitionNode.class); 
		dao.set("type", CompetitionNode.COMPETITION);
	...
	public static int calcCompetitionCount(Session session) throws Exception {
	...
	public Object[] loadCompetition() throws Exception{
		...
		if(pageNavigator != null && KNOWLEGE.equals(pageNavigator.getPageName())){
			...
			String title = locale.getString("$Competition") + " - " + getName();
	...
	public Object[] remove() throws Exception {
		...
		ICompetitionNode updateNode = (ICompetitionNode) sql(ICompetitionNode.class,	sb.toString());
	...
	public ModalWindow modify() throws Exception {
		...
		competitionNode.setId(this.getId());
		competitionNode.copyFrom(competitionNode.databaseMe());
		
		CompetitionTitle competitionTitle = new CompetitionTitle();
		competitionTitle.setTopicId(this.getId());
		competitionTitle.setTopicTitle(competitionNode.getName());
		competitionTitle.setTopicSecuopt("0".equals(competitionNode.getSecuopt()) ? false : true );
		competitionTitle.setMetaworksContext(new MetaworksContext());
		competitionTitle.getMetaworksContext().setWhen(MetaworksContext.WHEN_EDIT);
		competitionTitle.setLogoFile(new MetaworksFile());
		competitionTitle.session = session;
		return new ModalWindow(competitionTitle, MODIFY_POPUP_WIDTH, MODIFY_POPUP_HEIGHT, "$EditCompetition");
	...
	public ModalWindow exportHtml() throws Exception {
		CompetitionTitle competitionTitle = new CompetitionTitle();
		competitionTitle.setTopicId(this.getId());
		competitionTitle.setTopicTitle(this.getName());
		competitionTitle.getMetaworksContext().setHow(HTML);
		competitionTitle.getMetaworksContext().setWhen(MetaworksContext.WHEN_VIEW);
		
		competitionTitle.makeHtml();
				
		return new ModalWindow(competitionTitle, MODIFY_POPUP_WIDTH, MODIFY_POPUP_HEIGHT, "$ExportHtml");
	}
