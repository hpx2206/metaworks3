            ...
            if(metadata && metadata.fieldDescriptors){
                for (var j=0; j<metadata.fieldDescriptors.length; j++){
					...
					if( valiableObjectId ){
						...
						var buttonHtml = '<button class="btn btn-default btn-xs comment-btn" type="button" style="position:absolute; right:3px; top:50%; margin-top:-11px; display:none;" onClick="mw3.getFaceHelper('+objectId+').callReply(\''+fieldName+'\' , \''+valiableObjectId+'\')"><span class="glyphicon glyphicon-comment"></span> ' + mw3.localize('$AddComment') + '</button>';
