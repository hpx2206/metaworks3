...
import org.uengine.scheduler.AutoScheCronExp;
import org.uengine.scheduler.ManualScheCronExp;
import org.uengine.scheduler.ScheCronExp;
...
public class WaitActivity extends ReceiveActivity{
	...
	private static final String STOP_INSTANCE = "STOP_INSTANCE";
	private static final String PASS_INSTANCE = "PASS_INSTANCE";
	...
	public static void metaworksCallback_changeMetadata(Type type){
		 ...
		 * 
		 */
		fd = type.getFieldDescriptor("InstanceStop");
		fd.setDisplayName("인스턴스 종료");
		fd.setInputter(new RadioInput(
				new String[]{
						"stop Instance after wait give 'Date'",
						"no",
				}, new Object[]{
						STOP_INSTANCE, 
						PASS_INSTANCE, 
		...
		//metaworks 액비티비 배열 정의
		type.setFieldOrder(new String[]{
			"HowWait", "InstanceStop", "WaitUntil", "ScheCronExp", "StatusCode", "Cost", "ActivityIcon"	
		});
	...
	String instanceStop = PASS_INSTANCE;
	...
	String howWait;
		...
		public String getHowWait() {
			...
			if(howWait==null){
				setHowWait(isSchedule() ? WAITING_TYPE_PERIOD:WAITING_TYPE_PERIOD);
			}
...
//            }
	...
	/*
	 *인스턴스 종료 옵션 메소드
	 *스케쥴에서 세팅된 시간이 지나면 메소드 실행 
	 */
	
	public Boolean stopInstance(ProcessInstance instance) throws Exception {
		...
		try{
			...
			ProcessInstance theInstance = null;
			EndActivity endActivity = null;
			
			while(piDAO.next()){
				theInstance = ProcessInstance.create().getInstance(piDAO.getInstId().toString(), options);
				endActivity = new EndActivity();
				endActivity.setEscalate(false);
				endActivity.setStatus(EndActivity.STATUS_COMPLETED);
				endActivity.executeActivity(theInstance);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
