...
public class UpdateEventHandler implements ISVNEventHandler {
	...
	public UpdateEventHandler(SVNWindow cw){
		this.svnWindow = cw;
	}
    ...
    public void handleEvent(SVNEvent event, double progress) {
        ...
        if (action == SVNEventAction.UPDATE_ADD) {
        	...
        	svnWindow.addLog("Fetching external item into '"
                    + event.getFile().getAbsolutePath() + "'");
        	svnWindow.addLog("External at revision " + event.getRevision());
            return;
        } else if (action == SVNEventAction.UPDATE_COMPLETED) {
            /*
             * Updating the working copy is completed. Prints out the revision.
             */
        	svnWindow.addLog("At revision " + event.getRevision());
            return;
        } else if (action == SVNEventAction.ADD){
        	svnWindow.addLog("A     " + event.getURL());
            return;
        } else if (action == SVNEventAction.DELETE){
            svnWindow.addLog("D     " + event.getURL());
            return;
        } else if (action == SVNEventAction.LOCKED){
            svnWindow.addLog("L     " + event.getURL());
            return;
        } else if (action == SVNEventAction.LOCK_FAILED){
            svnWindow.addLog("failed to lock    " + event.getURL());
            return;
        }
        ...
        svnWindow.addLog(pathChangeType
