...
-- 2013_11_11 assessment (평가테이블 작성) HWS

delimiter $$

CREATE TABLE `assessment` (
  `completed` int(11) NOT NULL AUTO_INCREMENT,
...
-- 2013_11_11 enterprise(기업 가입시 정보 가지고 있는) table 작성 HWS

CREATE  TABLE `uengine`.`enterprise` (
  `enterprisename` VARCHAR(200) NOT NULL ,
  `representative` VARCHAR(100) NULL ,
  `businesspart` VARCHAR(100) NULL ,
  `address` VARCHAR(200) NULL ,
  `foundation` VARCHAR(100) NULL ,
  `number` VARCHAR(100) NULL ,
  `carryon` VARCHAR(100) NULL ,
  `fax` VARCHAR(100) NULL ,
  PRIMARY KEY (`enterprisename`) );
