var spaceTree = function(objectId, className){
	...
	var dataForJit = this.convert(originalData);
	
	
	var st = new $jit.ST({  
	
	    //id of viz container element  
	    injectInto: 'objDiv_' + objectId,  
	...
	st['spaceTreeHelper'] = this;

	//load json data  
	st.loadJSON(dataForJit);  
	//compute node positions and layout  
	st.compute();  
	//optional: make a translation of the tree  
	st.geom.translate(new $jit.Complex(-200, 0), "current");  
	//emulate a click on the root node.  
	st.setRoot(st.root, 'animate');  
 ...
 spaceTree.prototype.convert = function(originalData){
 
 	if(originalData.length){ //means array
 		var converted = [];
 		for(var i=0; i<originalData.length; i++){
 		
 			converted[i] = this.convert(originalData[i]);
 		}
	 	...
	 	var metadata = mw3.getMetadata(originalData.__className);
		...
		originalData['getDataForJit'] = function(fieldMapping){
			...
			dataForJit['id'] = originalData[fieldMapping.keyFd.name];
			dataForJit['name'] = originalData[fieldMapping.nameFd.name];
			dataForJit['data'] = originalData;
			
			if(originalData[fieldMapping.childrenFd.name]!=null)
				dataForJit['children'] = originalData[fieldMapping.childrenFd.name].getDataForJit(fieldMapping);
				
			return dataForJit;
		}
		
		var dataForJit = originalData.getDataForJit(fieldMapping);
