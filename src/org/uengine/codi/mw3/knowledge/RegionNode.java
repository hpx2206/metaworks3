...
import org.uengine.codi.mw3.model.IAssessment;
...
public class RegionNode extends Database<IRegionNode> implements IRegionNode {
	...
	public static IRegionNode load(Session session) throws Exception {
		...
		IRegionNode dao = (IRegionNode)MetaworksDAO.createDAOImpl(TransactionContext.getThreadLocalInstance(), sb.toString(), IRegionNode.class); 
		dao.set("type", "region");
	...
	public static IRegionNode moreView(Session session) throws Exception {
...
//		}
		...
		sb.append(" where id=?regionId");
		
		IRegionNode updateNode = (IRegionNode) sql(IRegionNode.class, sb.toString());
		updateNode.set("regionId", this.getId());
	...
	public ModalWindow modify() throws Exception {
		...
		regionTitle.setRegionId(this.getId());
		regionTitle.setRegionTitle(this.getName());
		regionTitle.setMetaworksContext(new MetaworksContext());
		regionTitle.getMetaworksContext().setWhen(MetaworksContext.WHEN_EDIT);
		regionTitle.session = session;
		return new ModalWindow(regionTitle, MODIFY_POPUP_WIDTH, MODIFY_POPUP_HEIGHT, "$RegionEdit");
	...
	public ModalWindow exportHtml() throws Exception {
		...
		regionTitle.setRegionId(this.getId());
		regionTitle.setRegionTitle(this.getName());
		regionTitle.getMetaworksContext().setHow(HTML);
		regionTitle.getMetaworksContext().setWhen(MetaworksContext.WHEN_VIEW);
		
		regionTitle.makeHtml();
				
		return new ModalWindow(regionTitle, MODIFY_POPUP_WIDTH, MODIFY_POPUP_HEIGHT, "$ExportHtml");
	}
