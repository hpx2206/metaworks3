...
//	}
	...
	public Popup detail() throws Exception {
		...
		try{
			...
			assessment.setEmpCode(getUserId());
			IAssessment iAssessment = assessment.findAvg();
			int size = iAssessment.size();
			int performance=0, integrity=0, reliability=0;
			
			// 사이즈가 0이 아니면 가져와서 사이즈 만큼 나누고
			if(size != 0) {
				while(iAssessment.next()){
					performance += iAssessment.getPerformance();
					integrity += iAssessment.getIntegrity();
					reliability += iAssessment.getReliability();
					
				}

				assessment.setPerformance(performance / size);
				assessment.setIntegrity(integrity / size);
				assessment.setReliability(reliability / size);
				assessment.setUserName(this.getUserId());
				setAssessment(assessment);
				
			// 아니면 디비에서 가져와서 세팅만 해준다.
			} else {
				while(iAssessment.next()){
					performance += iAssessment.getPerformance();
					integrity += iAssessment.getIntegrity();
